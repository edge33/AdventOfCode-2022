...
const getLowestCost = (
  ...
  const queue: { room: string; cost: number }[] = [{ room: start, cost: 0 }];
  while (queue.length) {
    const { room, cost } = queue.shift();
    if (room === end) {
      return cost;
    }
...
const getBestCombo = (
  ...
  const bestCombo = candidates.reduce((best, [path, score], i) => {
    ...
    console.log('best ', best, noOverlap[1] + score);
...
export default class ConcretePuzzle extends Puzzle {
  public solveFirst(): string {
    ...
    console.log(makeAllPaths(30, pathCosts, allValves).length);
...
(async () => {
  ...
  const puzzle = new ConcretePuzzle();
